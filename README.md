...
# 最新版本 2.2.4.0 —— 更新时间：2025年12月26日15点06分
...
#### [DT_20251226_2.2.4.0.apk，适合Android 8+系统](apk/daily/release/DT_20251226_2.2.4.0.apk)

1. 解决应用广播在Android 13以上版本无法收到的问题
2. 解决邮箱配置Session缓存导致邮件发送失败的问题
3. 优化远程指令【指令：`启动`】和【指令：`停止`】
4. 优化每日任务执行逻辑
5. 优化通知监听服务逻辑
6. 优化伪灭屏显示效果
7. 优化悬浮窗启动逻辑
8. 优化部分指令逻辑，避免因相同前缀导致指令错误执行
9. 增加3条指令——【指令：`考勤记录`】、【指令：`息屏`】、【指令：`亮屏`】
10. 增加手势开启伪灭屏【单手指从上到下滑动——开启，单手指从下到上滑动——关闭】，并支持选择是否开启，默认关闭
11. 区分单次任务（远程指令打卡）和多次任务（本机循环任务打卡）
...
1、先设置好自己打卡结果接收邮件的邮箱，至于”邮件标题“和”超时时间（跳转到钉钉之后停留在钉钉界面上的时间）“以及”任务口令“，那就随意了，可自行调整也可按默认的来。如下图：</br>
