...
| 2 | 远程"执行任务"指令 | 从另一台手机通过QQ、微信、支付宝或者TIM给自己发一条消息，内容包含"执行任务" | 触发 startTask，相当于点击了启动按钮                                                    | 任务是否开始执行                                   |
| 3 | 远程"终止任务"指令 | 任务执行中 → 发送内容含"终止任务"的通知                    | 任务停止                                                                       | 按钮恢复初始                                     |
| 4 | 远程"息屏"指令   | 发送内容含"息屏"的通知                              | MaskViewController 显示蒙层                                                    | 屏幕被遮罩覆盖、时钟动画启动                             |
| 5 | 远程"亮屏"指令   | 在息屏状态 → 发送内容含"亮屏"的通知                      | 蒙层消失                                                                       | 屏幕恢复可见                                     |
| 6 | 远程"截屏"指令   | 发送内容含"截屏"的通知（确保 MediaProjection 已就绪）      | 触发截屏 → 保存文件                                                                | 截图文件生成                                     |
...
| 3 | 远程"息屏/亮屏" | 发送"息屏"通知 → 等 3 秒 → 发送"亮屏"通知 | 蒙层按指令显示/隐藏                            | 与②联动的正确性              |
