...
| # | 场景         | 操作                                           | 预期结果                                                                       | 看什么                                        | 状态       |
|---|------------|----------------------------------------------|----------------------------------------------------------------------------|--------------------------------------------|----------|
| 1 | 打卡成功通知识别   | 让目标 APP 弹出"打卡成功"通知（如钉钉打卡成功）                  | NotificationMonitorService 捕获通知 → 发射 `ClockInSuccess` → MainActivity 收集到事件 | logcat 搜索 `ClockInSuccess`、`notifyClockIn` | ⚠️暂未测试   |
...
| # | 场景     | 操作                             | 预期结果                                                          | 看什么                    | 状态      |
|---|--------|--------------------------------|---------------------------------------------------------------|------------------------|---------|
| 1 | 修改重置时间 | TaskConfigActivity 修改重置时间 → 保存 | AlarmScheduler 取消旧闹钟 → 设置新闹钟 → FGService 更新 resetTickTime 倒计时 | 闹钟时间确实更新了、悬浮窗倒计时文字同步更新 | 🟢️测试通过 |
| 2 | 双重触发去重 | 模拟 Alarm + 每分钟广播几乎同时触发         | `ResetDailyTask` 被正确处理                                        | 不会启动两次调度、不出现重复任务执行     | ⚠️暂未测试  |
...
| 3 | 无网络发送  | 关闭 WiFi 和移动数据 → 触发消息发送           | 消息发送失败但不 Crash      | 超时后优雅失败、有错误日志 | ⚠️暂未测试  |
...
| 3 | 任务导入导出               | 导出当前任务列表为 JSON → 清空 → 重新导入                  | 导入后任务列表与导出前完全一致       | JSON 格式正确、所有字段完整                        | ⚠️暂未测试  |
| 4 | 并发读写                 | 快速多次切换页面、同时修改配置和任务                          | 数据一致性正确，无脏读脏写         | Room 事务正确、ReentrantReadWriteLock 不阻塞 UI | 🟢️测试通过 |
| 5 | SP 与 ConfigStore 一致性 | 修改邮箱 → 通过 SP 和 ConfigStore 各读一次             | 两套存储的值一致（或明确知道谁优先）    | 不出现"改了 A 但 B 读旧值"的情况                    | ⚠️暂未测试  |
